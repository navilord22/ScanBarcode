...
class _itemDetailsState extends State<itemDetails> {
  ...
  void _onLogoTap() {
    ...
    if (_tapCount == 3) {
      ...
      _tapTimer = Timer(const Duration(seconds: 5), _resetTapCount);
    }
  }
              ...
              onKey: (event) async {
                if (event.logicalKey.keyLabel == "Enter") {
                  ...
                  if (await widget.bloc.scanBarcode(tempBarcode)) {
                    ...
                    timer = Timer(const Duration(seconds: 10), //the wanted duration for the timer
                        () {
                      setState(() {
                        itemView = false;
                      });
  ...
  Widget itemWidget() {
    ...
    List<Widget> widgets = [];
    if (isDesktop(context) && product.defaultImage != "") {
      widgets = [
        itemImage(product),
        itemDetails(product)
      ];
  ...
  Widget itemImage(Product product) {
    ...
    if (product.defaultImage != "") {
      if (base64.hasMatch(product.defaultImage)) {
        temp = Card(
          elevation: 12,
